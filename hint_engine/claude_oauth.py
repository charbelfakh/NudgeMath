...
Ported from the rag-agent / ForgeStation flow: the user approves at claude.ai in
a browser tab, copies the one-time ``code#state`` string from the callback page,
and pastes it back. Tokens are stored in this app's own token file (under the
gitignored ``.nudgemath/`` dir) and refreshed transparently
(``grant_type=refresh_token``) shortly before expiry — no Claude Code CLI
involved anywhere.

The access token is consumed by :class:`hint_engine.llm_client.ClaudeSubscriptionClient`,
which calls the Anthropic Messages API directly with Bearer auth. This module
only manages the credential; it never solves problems and is unrelated to the
answer-blind generation boundary.
"""
...
_HTTP_HEADERS = {
    ...
    "User-Agent": "nudgemath/1.0",
}

# Single pending login per process — fine for a local-first app.
_pending: dict | None = None
# A pasted code is one-time and short-lived server-side; refuse to finish a
# sign-in started long ago so a stale tab fails with a clear message.
_PENDING_MAX_AGE_S = 30 * 60
...
def tokens_path() -> Path:
    """Where the subscription tokens live: ``NUDGEMATH_CLAUDE_OAUTH_TOKENS_PATH``
    or ``<repo>/.nudgemath/claude_oauth_tokens.json`` (gitignored)."""
    configured = os.getenv("NUDGEMATH_CLAUDE_OAUTH_TOKENS_PATH", "").strip()
    if configured:
        return Path(configured)
    return Path(__file__).resolve().parent.parent / ".nudgemath" / "claude_oauth_tokens.json"
...
def _store_tokens(payload: dict) -> dict:
    ...
    # A refresh response may omit refresh_token (no rotation); keep the previous
    # one in that case or every later refresh would silently become impossible.
...
def finish_login(pasted_code: str) -> dict:
    ...
    if time.time() - _pending["started_at"] > _PENDING_MAX_AGE_S:
        _pending = None
        raise ClaudeOAuthError("Sign-in expired — restart it and try again.")
